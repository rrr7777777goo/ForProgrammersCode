        ...
        while(a <= b)
        {
            for(int y = 0; y < dp[a].size(); y++)
            {
                for(int x = 0; x < dp[b].size(); x++)
                {
                    ...
                    if(ma[c1] == 0) { ma[c1] = i; dp[i].push_back(c1); }
                    if(ma[c2] == 0) { ma[c2] = i; dp[i].push_back(c2); }
                    if(ma[c3] == 0) { ma[c3] = i; dp[i].push_back(c3); }
                    if(c4 != -199999999) if(ma[c4] == 0) { ma[c4] = i; dp[i].push_back(c4); }
                    if(ma[c5] == 0 || ma[c5] > i) { ma[c5] = i; dp[i].push_back(c5); }
                    if(c6 != -199999999) if(ma[c6] == 0) { ma[c6] = i; dp[i].push_back(c6); }
                }
            }
            ++a; --b;
        }
